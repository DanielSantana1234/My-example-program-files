...
GRID_LINE_WIDTH = 4 # pixels
SYMBOL_WIDTH = WINDOW_SIZE/13 # pixels - adjust ratio
...
class Game(Tk):
    ...
    # placeholder title screen
        ...
        self.canvas.create_rectangle(
            int(WINDOW_SIZE/8), int(WINDOW_SIZE/8),
            int(WINDOW_SIZE*7/8), int(WINDOW_SIZE*7/8),
            width=int(WINDOW_SIZE/20),
            outline=X_COLOR)    

        self.canvas.create_rectangle(
            int(WINDOW_SIZE/6), int(WINDOW_SIZE/6),
            int(WINDOW_SIZE*5/6), int(WINDOW_SIZE*5/6),
            fill=X_COLOR,
            outline='')
        ...
        self.canvas.create_text(
            ...
            font=('Arial', int(-WINDOW_SIZE/12), 'bold'))
    ...
    def draw_O(self, grid_x, grid_y):
        ...
        x = self.gtpix(grid_x)
        y = self.gtpix(grid_y)
        delta = CELL_SIZE/1.5*SYMBOL_SIZE
